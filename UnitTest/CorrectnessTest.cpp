	...
	for (int level = startLevel; level < finishLevel; ++level)
	{
		...
		for (int j = 1; j < p.y_size; j++)
		{
			for (int i = 1; i < p.x_size; i++)
			{
				...
				//	ASSERT_TRUE(gpu->f[c] == f) << "c = " << c << std::endl;
				//ASSERT_TRUE(gpu->s[c] == s) << "c = " << c << std::endl;
			}
		}
		delete gpu;
	}
